...
namespace Postmaster.Api
{
    ...
    public class DiskMessageStore : IMessageStore
    {
        ...
        /// <inheritdoc />
        public async Task RouteUserToUserMessageAsync(string senderId, string recipientId, Message message)
        {
            if (string.IsNullOrEmpty(senderId)) throw new ArgumentNullException(nameof(senderId));
            if (string.IsNullOrEmpty(recipientId)) throw new ArgumentNullException(nameof(recipientId));
            if (message == null) throw new ArgumentNullException(nameof(message));

            // Store in sender's outbound
            await StoreUserMessageAsync(senderId, message, false);
            
            // Store in recipient's inbound
            await StoreUserMessageAsync(recipientId, message, true);
        }
